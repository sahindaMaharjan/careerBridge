...
namespace careerBridge.Services
{
    public class JobSearchService
    {
        ...
        private const string apiKey = "5368558acemshdffde0a1cf12249p1d0d9ejsn9fc47e8eb1c3";  // Put your RapidAPI key here
        ...
        public async Task<string> SearchJobsAsync(string query)
        {
            ...
            var response = await _httpClient.GetAsync(url); using System.Net.Http;
            using System.Threading.Tasks;
...
namespace careerBridge.Services
    {
        ...
        {
            private readonly HttpClient _httpClient;
            private const string apiKey = "YOUR_RAPIDAPI_KEY";  // Replace with your actual key
            private const string apiHost = "jsearch.p.rapidapi.com";
            private const string baseUrl = "https://jsearch.p.rapidapi.com/search";
            ...
            public async Task<string> SearchJobsAsync(string query, string location = "", int? posted = null, int? minSalary = null)
            {
                ...
                if (posted.HasValue)
                    url += $"&date_posted={posted.Value}"; // RapidAPI may use different param, check docs
